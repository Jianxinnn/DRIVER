...
def run_mmseqs2(x, prefix, use_env=True, use_filter=True,
  ...
  unique_prefix = os.path.join(prefix, str(uuid.uuid4()))
  ...
  if use_templates:
    ...
    for k,TMPL in templates.items():
      TMPL_PATH = f"{unique_prefix}_{mode}/templates_{k}"
...
def main():
  parser = argparse.ArgumentParser(description="Query MMseqs2 and write A3M alignments")
  parser.add_argument('--split', type=str)
  parser.add_argument('--wt', type=str, default=None)
  parser.add_argument('--entry', type=str, default=None)
  parser.add_argument('--outdir', type=str, default='./output/msa_queries')
  ...
  if args.split:
    ...
    assert args.entry is not None and args.wt is not None, 'Please provide a sequence'
    query_seq = [args.wt]
    names = [args.entry]

    os.makedirs(args.outdir, exist_ok=True)
    msas = run_mmseqs2(query_seq, prefix='/tmp/', user_agent='bjing2016/alphaflow')
    os.system('rm -rf /tmp/_env')

    for name, msa in zip(names, msas):
      with open(f'{args.outdir}/{name}.a3m', 'w') as f:
          f.write(msa)
