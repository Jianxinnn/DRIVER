...
def _load_structure_biopdb(fpath, chain=None):
    """Load structure via Bio.PDB (supports .pdb and .cif) and return
    a list of residues (standard only) and their backbone coords.
    Returns
        coords: np.ndarray [L, 3, 3] for N, CA, C (nan if missing)
        seq: str of length L
    """
    parser = MMCIFParser(QUIET=True) if fpath.endswith("cif") else PDBParser(QUIET=True)
    structure = parser.get_structure("protein", fpath)

    # Determine chains to use
    if chain is None:
    ...
    for model in structure:
        for ch in model:
            ...
            for residue in ch:
                ...
                if residue.get_id()[0] != " ":
                    continue
                # collect backbone N, CA, C; fill nan if missing
                atom_names = ["N", "CA", "C"]
                atoms_xyz = []
                for an in atom_names:
                    if an in residue:
                        atoms_xyz.append(residue[an].get_coord().tolist())
                    else:
                        atoms_xyz.append([float("nan"), float("nan"), float("nan")])
                coords_list.append(atoms_xyz)
                seq_list.append(seq1(residue.get_resname()))

    if len(coords_list) == 0:
        raise ValueError("No valid residues found after filtering. Check chain id or file integrity.")
    import numpy as _np
    return _np.asarray(coords_list, dtype=float), "".join(seq_list)


def generate_graph(pdb_file, max_distance=10, chain="A"):
    """
    Generate graph data from structure file (supports .pdb and .cif).

    Params:
        pdb_file: path to a PDB or mmCIF file
        max_distance: edge cutoff (Angstrom)

    Returns:
        torch_geometric.data.Data with node/edge features and distance matrix
    """
    # Load via Bio.PDB to avoid biotite dependency at runtime
    coords_np, one_letter_seq = _load_structure_biopdb(pdb_file, chain=chain)

    # Convert to torch and sanitize missing values
    ...
    # Distance-based edges computed in numpy for down-stream numpy ops
    ca_np = ca_coords.cpu().numpy()
    distances = spa.distance_matrix(ca_np, ca_np)
    edge_index = torch.tensor(np.array(np.where(distances < max_distance)), dtype=torch.long)
    # Remove self-loops
    mask_edge = edge_index[0] != edge_index[1]
    ...
    data = Data(
        ...
        edge_s=edge_s,
        edge_v=edge_v,
        distances=distances,  # kept as numpy for generate_pos_subgraph()
...
def process_pdb_file(
    ...
    try:
        graph = generate_graph(pdb_file, max_distance, chain=chain)
...
class PdbQuantizer:
    ...
    def __init__(
        ...
        # Safer loading of weights with fallback for older torch versions
        try:
